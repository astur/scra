...
module.exports = (options, callback) => {
    ...
    const req = conf.request(conf.httpOptions, res => {
        ...
        res.on('end', () => {
            ...
            res.options = options;
            try {
                const wrappedResponse = wrapResponse(conf, res, chunks);
                callback(null, wrappedResponse);
            } catch(e){
                callback(e);
            }
        });
    ...
    req.on('abort', () => {
        callback(new TimeoutError(conf.url, conf.timeout, timings, timingPhases));
    });

    req.on('error', err => {
        stopTimer();
        callback(new NetworkError(conf.url, err, timings, timingPhases));
    });
